...
std::string &Utils::generate_spherical_xml (
  SpatialMedia::Parser::projection projection,
  SpatialMedia::Parser::enMode stereo,
  std::string stitchingSoftware,
  int *crop
) {
  // Configure inject xml
  static std::string empty;

  int iSize = SPHERICAL_XML_CONTENTS.length() + stitchingSoftware.length();
  char *buffer = new char[iSize];
  sprintf(buffer, SPHERICAL_XML_CONTENTS.c_str(), stitchingSoftware.c_str());
  std::string spherical_xml(buffer);
  delete buffer;

  if (projection == SpatialMedia::Parser::EQUIRECT)
    spherical_xml += SPHERICAL_XML_CONTENTS_EQUIRECT;
  if (projection == SpatialMedia::Parser::CUBEMAP)
    spherical_xml += SPHERICAL_XML_CONTENTS_CUBEMAP;
  if (projection == SpatialMedia::Parser::SINGLE_FISHEYE)
